...
public class WIP_OQC_QCANALYSIS extends Activity {
	...
	String msProductOrderId = "", msProductId, msProductName, msProductModel, msProductCompId, msProductSerialNumber,
			msStepId, msBomflag = "", msRepeter = "", msEqpId, msAnalysisformsID = "", msSampletimes = "", msStepSEQ,
			msSUPPLYLOTID, msSUPPLYID, msQC_ITEM, msQCTYPE, msCustomer;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			editSumCount = (EditText) findViewById(R.id.wipoqc_tvSumCount);
			editInCount = (EditText) findViewById(R.id.wipoqc_tvInCount);
							...
							if (lsAnalysisTable.get(milv1RowNum).get("ISJUDGE").toString().trim().equals("Y")) {
								if (!edit.getText().toString().trim().equals("")) {
									...
									if (lsAnalysisTable.get(milv1RowNum).get("SPECMINVALUE").toString().equals("")) {
										...
										sminValueString = lsAnalysisTable.get(milv1RowNum).get("SPECMINVALUE")
												.toString();
									}
									if (lsAnalysisTable.get(milv1RowNum).get("SPECMAXVALUE").toString().equals("")) {
										smaxValueString = "9999999999";
									} else {
										smaxValueString = lsAnalysisTable.get(milv1RowNum).get("SPECMAXVALUE")
												.toString();
									}

									if (Double.parseDouble(edit.getText().toString().trim()) >= Double
											.parseDouble(sminValueString)
											&& Double.parseDouble(edit.getText().toString().trim()) <= Double
													.parseDouble(smaxValueString)) {
				...
				public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
					...
					try {
						...
						if (sBid.value.equals("预设")) {
							sResult = db.GetERP_ASSEMBLESPECIFICATION(msProductId, msProductCompId, msProductOrderId,
									"预设", "", params.get("StepID"), lsSid);
						} else {
							sResult = db.GetERP_ASSEMBLESPECIFICATION(msProductId, msProductCompId, msProductOrderId,
									"", sBid.value, params.get("StepID"), lsSid);
						}
						if (sResult != "") {
							MESCommon.showMessage(WIP_OQC_QCANALYSIS.this, sResult);
							finish();
			...
			lv0.setOnItemClickListener(new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
					try {
						...
						holder.cb.toggle();
						// 将CheckBox的选中状况记录下来
						wipoqcAdapterTab0.getIsSelected().put(position, holder.cb.isChecked());
					...
					if (keyCode == KeyEvent.KEYCODE_ENTER && event.getAction() == KeyEvent.ACTION_DOWN) {
						...
						try {
							...
							if (editLCID.getText().toString().length() > 0
								...
								AlertDialog alert = new AlertDialog.Builder(WIP_OQC_QCANALYSIS.this).setTitle("确认")
							...
							if (editLCID.getText().toString().equals("")) {
								...
								for (int i = 0; i < lsLcSeq.size(); i++) {
									...
									mapItnbrSeq.put(lsLcSeq.get(i).get("trseq").toString(),
											lsLcSeq.get(i).get("itnbr").toString());
								}

								editSumCount.setText(String.valueOf(sumCount));// LC总出货数
								...
								editInCount.setText(lsInCount.get(0).get("InCount").toString());// LC扫描出货数
								ArrayAdapter<SpinnerData> Adapter1 = new ArrayAdapter<SpinnerData>(
										WIP_OQC_QCANALYSIS.this, android.R.layout.simple_spinner_item, lst);
								...
								msLcSeq = LcSeq.text;
								lsCompID.clear();
								sResult = db.GetProductSerialNumber(txtInput.getText().toString().trim(), "", "", "QF",
										"制造号码", "装配", lsCompID);

								if (lsCompID.size() <= 0) {
									MESCommon.show(WIP_OQC_QCANALYSIS.this,
											"物料条码【" + txtInput.getText().toString().trim() + "】， 不存在!");
									return false;
								} else {
									lsProcess.clear();
									String sOrderID = lsCompID.get(0).get("PRODUCTORDERID").toString();
									sResult = db.GetProductProcess(sOrderID, txtInput.getText().toString().trim(),
											lsProcess);
									if (sResult != "") {
										MESCommon.showMessage(WIP_OQC_QCANALYSIS.this, sResult);
										finish();
									...
									if (lsProcess.size() == 0) {
										MESCommon.show(WIP_OQC_QCANALYSIS.this,
												" 制造号码【" + txtInput.getText().toString().trim() + "】，没有设定生产工艺流程！");
										Clear();
										return false;
									}
									...
									String sError = "";
									lsProduct.clear();
									sSql = " SELECT * FROM MPRODUCT WHERE PRODUCTID ='"
											+ lsProcess.get(0).get("PRODUCTID").toString() + "'  ";
									...
									if (!lsProduct.get(0).get("PRODUCTID").toString()
											.equals(mapItnbrSeq.get(msLcSeq))) {
										MESCommon.show(WIP_OQC_QCANALYSIS.this,
												"制造号码【" + txtInput.getText().toString().trim() + "】的件号和LC序号对应的件号不一致！");
										return false;
									}
									...
									for (int i = 0; i < lsCompTable.size(); i++) {
										if (txtInput.getText().toString().trim()
												.equals(lsCompTable.get(i).get("SEQ").toString())
												&& msLcSeq.equals(lsCompTable.get(i).get("ISSP").toString())) {
											MESCommon.show(WIP_OQC_QCANALYSIS.this,
													"条码[" + lsCompTable.get(i).get("SEQ").toString() + "] 已绑定,LC序号["
															+ lsCompTable.get(i).get("ISSP").toString() + "]!");
											return false;

										}
										if (lsCompTable.get(i).get("ISSP").toString().equals(msLcSeq)) {
											InLCcount+=1;
										}
									...
									//是不是到达最大个数
									lsLcComp.clear();
									sSql = " SELECT COUNT(*) AS INCOUNT FROM HZ_OQC_QCANALYSIS WHERE LCNO='"+editLCID.getText().toString()+"' AND LCSEQ ='" + msLcSeq+ "'  ";
									sError = db.GetData(sSql, lsLcComp);
									if (sError != "") {
										MESCommon.showMessage(WIP_OQC_QCANALYSIS.this, sError);
										return false;
									}			
									...
									//是不是被该LC单号绑定
									lsLcComp.clear();
									sSql = " SELECT * FROM HZ_OQC_QCANALYSIS WHERE LCNO='"+editLCID.getText().toString()+"' AND PRODUCTCOMPID ='" + msProductCompId+ "'  ";
									sError = db.GetData(sSql, lsLcComp);
									if (sError != "") {
										MESCommon.showMessage(WIP_OQC_QCANALYSIS.this, sError);
										return false;
									...
									if (lsLcComp.size() > 0) {
										MESCommon.show(WIP_OQC_QCANALYSIS.this,
												"条码[" + msProductCompId+ "] 已绑定,该LC单号!");
										return false;
									}

									// 查询制造号码是否被出货过
									lsLcComp.clear();
									sSql = " SELECT * FROM HZ_OQC_QCANALYSIS WHERE PRODUCTCOMPID ='" + msProductCompId
											+ "'  ";
									sError = db.GetData(sSql, lsLcComp);
									if (sError != "") {
										MESCommon.showMessage(WIP_OQC_QCANALYSIS.this, sError);
									...
									if (lsLcComp.size() > 0) {
										AlertDialog alert = new AlertDialog.Builder(WIP_OQC_QCANALYSIS.this)
												.setTitle("确认")
												.setMessage("条码[" + msProductCompId + "] 已绑定,LC单号["
														+ lsLcComp.get(0).get("LCNO").toString() + "]!" + ",是否继续加入!")
												.setPositiveButton("确定", new DialogInterface.OnClickListener() {
													@Override
													public void onClick(DialogInterface dialog, int which) {
														exeLstCompTable();
	...
	public static class wipoqcAdapterTab0 extends BaseAdapter {
		...
		public wipoqcAdapterTab0(List<HashMap<String, String>> items, Context context) {
			this.items = items;
	...
	public static class wipoqcAdapter extends BaseAdapter {
		...
		public wipoqcAdapter(List<HashMap<String, String>> items, Context context) {
			this.items = items;
			this.context = context;
			inflater = LayoutInflater.from(context);
	...
	public static class wipoqcAdapterTab2 extends BaseAdapter {
		...
		public wipoqcAdapterTab2(List<HashMap<String, String>> items, Context context) {
			this.items = items;
			this.context = context;
			inflater = LayoutInflater.from(context);
	...
	public String checkExist(String number) {
		...
		try {
			...
			String sSQL = "SELECT PRODUCTSERIALNUMBER AS NUMBER ,PRODUCTCOMPID FROM PROCESS_STEP_P WHERE PRODUCTSERIALNUMBER= '"
					+ number + "'  ";
			...
			if (lscheckExist.size() > 0) {
				return sResult = "扫描条码【" + number + "】,已经被【" + lscheckExist.get(0).get("PRODUCTCOMPID").toString()
						+ "】组装装配过";
			}
			// PRODUCTSERIALNUMBER和——p不一样表示不是次组立的轴承座！提示被次组立装配使用了。
			sSQL = "SELECT SERIALNUMBER_P  AS NUMBER, PRODUCTSERIALNUMBER_OLD  FROM PROCESS_STEP_PF WHERE SERIALNUMBER_P= '"
					+ number + "' and  PRODUCTSERIALNUMBER!=''   ";
			...
			if (lscheckExist.size() > 0) {
				return sResult = "扫描条码【" + number + "】,已经被【"
						+ lscheckExist.get(0).get("PRODUCTSERIALNUMBER_OLD").toString() + "】次组立装配过";
			}
			return sResult;
		} catch (Exception e) {
			// TODO: handle exception
